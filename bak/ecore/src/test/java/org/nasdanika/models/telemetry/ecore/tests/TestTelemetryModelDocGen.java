package org.nasdanika.models.telemetry.ecore.tests;
...
import org.nasdanika.models.telemetry.ecore.EcoreGenTelemetryProcessorsFactory;
 ...
 *
 */
public class TestTelemetryModelDocGen {
	
	@Test
	public void testGenerateTelemetryModelDoc() throws IOException, DiagnosticException {
		ProgressMonitor progressMonitor = new NullProgressMonitor(); // new PrintStreamProgressMonitor();
		MutableContext context = Context.EMPTY_CONTEXT.fork();
		Consumer<Diagnostic> diagnosticConsumer = d -> d.dump(System.out, 0);
		List<Function<URI,Action>> actionProviders = new ArrayList<>();		
		EcoreGenTelemetryProcessorsFactory ecoreGenTelemetryProcessorFactory = new EcoreGenTelemetryProcessorsFactory(context);		
		EcoreNodeProcessorFactory ecoreNodeProcessorFactory = new EcoreNodeProcessorFactory(
				...
				diagnosticConsumer,
				ecoreGenTelemetryProcessorFactory);
		
		File actionModelsDir = new File("target\\action-models\\");
		actionModelsDir.mkdirs();
		File output = new File(actionModelsDir, "telemetry.xmi");
		
		
		Map<EPackage, URI> packageURIMap = Map.ofEntries(
				Map.entry(EcorePackage.eINSTANCE, URI.createURI("https://ecore.models.nasdanika.org/")),	
				Map.entry(ChangePackage.eINSTANCE, URI.createURI("https://change.models.nasdanika.org/"))	
			);
			
		EcoreHtmlAppGenerator eCoreHtmlAppGenerator = new EcoreHtmlAppGenerator(
				org.nasdanika.telemetry.model.ModelPackage.eINSTANCE, 
				packageURIMap, 
		...
		String siteMapDomain = "https://telemetry.models.nasdanika.org";		
